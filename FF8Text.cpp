...
QByteArray FF8Text::toFF8() const
{
	...
	for(qsizetype c = 0; c < stringSize; ++c)
	{
		...
		// =========================
		// UPDATED NEWLINE HANDLING
		// =========================
		if(comp=='\n' || comp=='\r') {
			// Handle Windows CRLF safely
			if(comp=='\r' && c+1<stringSize && string.at(c+1)=='\n') {
				c += 1;
			}

			// Do not emit a normal newline before {NewPage}
			if(string.mid(c+1, 9).compare("{NewPage}", Qt::CaseInsensitive) != 0) {
				ff8str.append('\x02'); // normal newline
			}
			...
			// =========================
			// ADDED EXPLICIT {NewPage} TOKEN SUPPORT
			// =========================
			if(rest.startsWith("{NewPage}", Qt::CaseInsensitive)) {
				ff8str.append('\x01');
				c += 9;

				// Skip newline immediately after {NewPage}
				if(c + 1 < stringSize && (string.at(c + 1) == '\n' || string.at(c + 1) == '\r')) {
					++c;

					// Handle CRLF after token
			...
			for(quint8 i=0 ; i<11 ; ++i)
				...
				if(c + 1 < stringSize && string.at(c+1)=='x') {
